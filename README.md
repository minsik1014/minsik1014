<div align="center">

<!-- 타이핑 애니메이션 -->
[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1000&color=3B82F6&center=true&vCenter=true&width=500&lines=Hi+there!+I'm+Minsik+Sim+👋;Frontend+Developer;Building+with+FSD+Architecture)](https://git.io/typing-svg)

**"사용자의 눈에 즐겁고, 동료가 읽기에 편한 코드를 작성합니다."**

[![Portfolio](https://img.shields.io/badge/Portfolio-Pokedex_Style-FF4B4B?style=for-the-badge&logo=React&logoColor=white)](https://my-portfolio-zeta-ashy-62.vercel.app/)
[![Email](https://img.shields.io/badge/Email-ms03zzang@naver.com-03C75A?style=for-the-badge&logo=Naver&logoColor=white)](mailto:ms03zzang@naver.com)

</div>

---

### 💡 About Me
- 🎓 **동양미래대학교** 컴퓨터소프트웨어공학과 (22학번)
- 🏆 **UMC 9기 데모데이 우수상 수상** (73개 팀 중 선정)
- 🗣️ **동양미래대학교 컴퓨터소프트웨어공학과 학생회 소통부장** (원활한 소통과 팀 빌딩을 중시합니다)
- 🛠 **Focus**: **Feature-Sliced Design (FSD)** 아키텍처를 적극 도입하여, 대규모 도메인에서도 흔들리지 않는 확장성과 유지보수성을 확보하는 프론트엔드 개발을 지향합니다.

---

### 📊 GitHub Status & Activity

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=minsik1014&show_icons=true&theme=transparent&hide_border=true&title_color=3B82F6&icon_color=3B82F6" width="48%" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=minsik1014&layout=compact&theme=transparent&hide_border=true&title_color=3B82F6" width="48%" />
</div>

---

### 🛠 Tech Stack

**Frontend** <br>
<img src="https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB"/>
<img src="https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white"/>
<img src="https://img.shields.io/badge/Vite-646CFF?style=flat-square&logo=vite&logoColor=white"/>
<img src="https://img.shields.io/badge/TailwindCSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white"/>
<img src="https://img.shields.io/badge/Zustand-443E38?style=flat-square&logo=React&logoColor=white"/>
<img src="https://img.shields.io/badge/TanStack_Query-FF4154?style=flat-square&logo=React&logoColor=white"/>

**Backend & Data** <br>
<img src="https://img.shields.io/badge/Java-007396?style=flat-square&logo=java&logoColor=white"/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/MariaDB-003545?style=flat-square&logo=mariadb&logoColor=white"/>

---

### 🚀 Featured Projects

#### 🌐 [Gusring (구스링) - 금천구 리빙랩 프로젝트](https://github.com/minsik1014/gusring) _(진행 중)_
**다국어 민원 안내 및 거주 지원 플랫폼**
- **Role**: 프론트엔드 리드 (Team EL)
- **Point**: FSD 아키텍처를 도입하여 복잡한 다국어 서비스와 거주 지원 도메인을 효과적으로 분리하고 확장 가능한 구조로 설계 중입니다.

#### 🎓 [MOEASY] _(진행 중)_
**동아리 및 소모임 관리 플랫폼 (졸업 작품)**
- **Point**: 사용자 친화적인 UI/UX를 기획하고, 동아리 운영에 필요한 복잡한 요구사항을 관리하기 쉬운 플랫폼으로 구현하고 있습니다.

#### 🎮 [PlayProof](https://github.com/Playproof-Umc/Playproof-Frontend)
**게임 팀워크 신뢰 기반 커뮤니티 플랫폼**
- **Role**: 프론트엔드 팀장
- **Point**: 230회 이상의 커밋. 실시간 채팅 및 스케줄러 도메인 완성. UMC 9기 **우수상** 수상.

#### 🛡️ [GLife](https://github.com/minsik1014/GLife_EXPO_React)
**산업안전교육 관리자 대시보드**
- **Role**: 프론트엔드 아키텍처 및 인증 시스템 설계
- **Point**: JWT Refresh Token 플로우 및 보호 라우팅 설계, 엑셀 데이터 활용 대시보드 UI 구축.

---

<div align="center">
  <!-- 방문자 수 뱃지 -->
  <img src="https://hits.seeyoufarm.com/api/count/bnd/badge.svg?url=https%3A%2F%2Fgithub.com%2Fminsik1014&title=Profile+Views&edge_flat=false"/>
</div>
